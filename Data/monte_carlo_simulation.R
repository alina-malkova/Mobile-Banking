#' ==============================================================================
#' Monte Carlo Simulation: When K Matters
#'
...
#'
#' Vectorized EM implementation. Runtime: ~3-7 hours depending on hardware.
#'
...
cat("============================================================\n")
cat("MONTE CARLO SIMULATION: WHEN K MATTERS\n")
...
R_values <- c(100, 50, 10)   # Fewer reps for larger N
...
cf_stats <- results_all %>%
  ...
  summarise(cf_mean = mean(cf_effect, na.rm=TRUE),
            cf_sd = sd(cf_effect, na.rm=TRUE), .groups="drop")
...
cf_range <- results_all %>%
  ...
  summarise(mean_range = mean(cf_range, na.rm=TRUE),
            median_range = median(cf_range, na.rm=TRUE), .groups="drop")
...
write.csv(results_all, file.path(DATA_DIR, "output/monte_carlo_results.csv"), row.names=FALSE)
write.csv(k_freq, file.path(DATA_DIR, "output/monte_carlo_k_selection.csv"), row.names=FALSE)
write.csv(cf_stats, file.path(DATA_DIR, "output/monte_carlo_cf_stats.csv"), row.names=FALSE)
write.csv(cf_range, file.path(DATA_DIR, "output/monte_carlo_cf_range.csv"), row.names=FALSE)
cat("\nCSV results saved to Data/output/\n")
...
tryCatch({
  ...
  ggsave(file.path(OUTPUT_DIR, "fig_mc1_k_selection_freq.pdf"), fig_mc1, width=8, height=5)
  ggsave(file.path(OUTPUT_DIR, "fig_mc1_k_selection_freq.png"), fig_mc1, width=8, height=5, dpi=300)
  cat("  Saved: fig_mc1\n")
...
tryCatch({
  ...
  fig_mc2 <- cf_by_k %>% filter(N == 10000) %>%
    ...
    theme_minimal(base_size = 12)
  ggsave(file.path(OUTPUT_DIR, "fig_mc2_cf_by_design.pdf"), fig_mc2, width=8, height=5)
  ggsave(file.path(OUTPUT_DIR, "fig_mc2_cf_by_design.png"), fig_mc2, width=8, height=5, dpi=300)
  cat("  Saved: fig_mc2\n")
...
tryCatch({
  ...
  fig_mc3 <- cf_range_dist %>% filter(N == 10000) %>%
    ...
    labs(title = "Distribution of CF Range Across K",
         x = "Range (pp)", y = "Count") +
    theme_minimal(base_size = 12)
  ggsave(file.path(OUTPUT_DIR, "fig_mc3_cf_range_dist.pdf"), fig_mc3, width=8, height=5)
  ggsave(file.path(OUTPUT_DIR, "fig_mc3_cf_range_dist.png"), fig_mc3, width=8, height=5, dpi=300)
  cat("  Saved: fig_mc3\n")
...
tryCatch({
  ...
  fig_mc4 <- bic_acc %>%
    ...
    theme_minimal(base_size = 12)
  ggsave(file.path(OUTPUT_DIR, "fig_mc4_bic_accuracy.pdf"), fig_mc4, width=8, height=5)
  ggsave(file.path(OUTPUT_DIR, "fig_mc4_bic_accuracy.png"), fig_mc4, width=8, height=5, dpi=300)
  cat("  Saved: fig_mc4\n")
