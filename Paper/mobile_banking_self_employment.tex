...
\begin{abstract}
\noindent This paper investigates whether mobile banking can substitute for traditional branch banking in supporting self-employment. Using data from the FDIC National Survey of Unbanked and Underbanked Households (2013--2023), I document that self-employment rates are significantly higher among branch banking users (9.95\%) compared to mobile-only users (7.19\%). I develop a dynamic structural model of joint banking mode and employment choice estimated via the Arcidiacono-Miller CCP approach with finite dependence and unobserved heterogeneity. Following recent econometric advances, I employ a three-pronged approach to select the number of latent types: Hao-Kasahara (2025) panel BIC, Bonhomme-Lamadon-Manresa (2022) counterfactual stability, and Budanova (2025) penalized MLE. All three methods support $K=4$ types with heterogeneous responses to branch closures, including a substantial group (24\%) highly dependent on branch-based relationship lending. Counterfactual simulations show that a 50\% reduction in branch access reduces aggregate self-employment by approximately 11\%, with meaningful sensitivity to unobserved heterogeneity assumptions (effects range from 1\% to 11\% across specifications). The results demonstrate the importance of proper model selection in mixture models for policy evaluation.
...
Following \citet{arcidiacono2011}, I extend the model to incorporate unobserved heterogeneity via a finite mixture. Selecting the number of types $K$ in mixture models is a challenging econometric problem because standard likelihood ratio tests have non-standard distributions when parameters are on the boundary. I employ a three-pronged approach to model selection following recent developments in the literature:

\begin{enumerate}
    \item \textbf{Hao-Kasahara Panel BIC} \citep{hao2025}: The standard BIC uses $\ln(N)$ where $N$ is the number of observations. For panel data with repeated cross-sections, \citet{hao2025} show that consistent order selection requires adjusting for the panel structure: Panel BIC $= -2\ln\mathcal{L} + p \cdot \ln(N_{panels}) \cdot c(T)$, where $c(T) = 1 + 1/T$ corrects for the number of time periods.
...
The four BIC-selected types are economically interpretable:

\begin{itemize}
    \item \textbf{Type 1 (9.5\%)}: No branch effect (coefficient near zero). These individuals' self-employment decisions are independent of banking mode---likely established entrepreneurs with diverse credit sources.

    \item \textbf{Type 2 (15.2\%)}: Negative branch effect ($\beta = -0.030$, $t = -9.6$). Counterintuitively, these individuals are \textit{less} likely to be self-employed when using branch banking, suggesting selection of risk-averse individuals into traditional banking.

    \item \textbf{Type 3 (25.9\%)}: Moderate positive branch effect ($\beta = 0.026$, $t = 5.5$). Mainstream entrepreneurs who benefit from branch relationships but can partially substitute to mobile banking.

    \item \textbf{Type 4 (24.3\%)}: Large positive branch effect ($\beta = 0.138$, $t = 8.8$). Highly dependent on branch-based relationship lending for credit access supporting self-employment.
\end{itemize}
...
Table \ref{tab:counterfactual} presents counterfactual predictions from the structural model. Critically, the counterfactual choice probabilities are derived from re-solving the model under changed branch density---they are \textit{not} imposed exogenously. When branch density falls, individuals re-optimize their joint banking-employment choices given the new environment, and the model predicts the redistribution across all nine alternatives. This is the key advantage of the structural approach.

The counterfactual mechanism operates through the utility function. When branch density declines by 50\%, the utility of self-employment for branch banking users falls (due to reduced credit access via the $\gamma_{branch \times density}$ interaction). This causes substitution along two margins:
\begin{enumerate}
    \item \textbf{Extensive margin}: Some individuals switch banking modes (from branch to mobile/unbanked)
    \item \textbf{Intensive margin}: Among those who remain branch users, some switch from self-employment to wage employment
\end{enumerate}

Both margins are captured by the multinomial choice model---the transition probabilities emerge from the model rather than being assumed.

\begin{table}[H]
\centering
\caption{Structural Counterfactual Policy Simulations}
\label{tab:counterfactual}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
Scenario & SE Rate & Change & \% Change & Source \\
\midrule
\textbf{Baseline} & 10.58\% & -- & -- & Data \\
\midrule
\multicolumn{5}{l}{\textit{Structural Counterfactual (Model-Predicted Transitions)}} \\
\quad 25\% density reduction & 4.17\% & $-6.41$ pp & $-60.6\%$ & Model \\
\quad 50\% density reduction & 1.85\% & $-8.73$ pp & $-82.5\%$ & Model \\
\midrule
\multicolumn{5}{l}{\textit{Comparison: Assumed Transition Approach}} \\
\quad 50\% reduction (80/20 split) & 9.66\% & $-0.92$ pp & $-8.7\%$ & Assumed \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: Structural counterfactuals derive choice probabilities from the estimated logit model: $P(SE|X, density') = \Lambda(X'\hat{\beta} + \hat{\gamma}_{branch \times density} \times branch \times density')$. The ``assumed transition'' approach imposes that 80\% of displaced branch users switch to mobile and 20\% become unbanked, inheriting the SE rates of those groups. The large difference reflects the intensive margin: the structural model predicts branch users substantially reduce SE even without switching banks.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{enumerate}
    \item \textbf{Structural counterfactuals differ dramatically from assumed transitions}: The model-predicted effect of 50\% density reduction is an 82.5\% decline in self-employment---nearly ten times larger than the 8.7\% decline implied by the commonly-used assumed transition approach. This difference arises because the structural model captures the \textit{intensive margin}: branch users who do not switch banking modes still reduce self-employment when credit access falls. The assumed transition approach misses this channel entirely.

    \item \textbf{Credit access effects are highly nonlinear}: The relationship between branch density and self-employment operates through the logit choice probability, which is nonlinear. Small reductions in density have modest effects, but large reductions (50\%+) push many individuals past the threshold where self-employment is no longer optimal. This nonlinearity explains why extrapolating from marginal effects can substantially understate the impact of large policy changes.

    \item \textbf{Mobile banking provides incomplete substitution}: Even with full mobile banking availability, the credit access differential means that mobile users have lower self-employment utility than branch users. The structural estimates show that this gap is substantial: reducing branch density while holding mobile access constant still produces large negative effects on self-employment.
\end{enumerate}

\textbf{Important caveat}: The large structural effects (60--80\%) involve extrapolation beyond the observed variation in branch density. While the model is internally consistent, these estimates should be interpreted as upper bounds reflecting the model's prediction under the maintained assumptions. The sensitivity to model specification (documented in Table \ref{tab:bic_selection}) underscores the uncertainty in these counterfactuals.
