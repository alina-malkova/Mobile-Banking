...
\begin{abstract}
\noindent This paper investigates whether mobile banking can substitute for traditional branch banking in supporting self-employment. Using data from the FDIC National Survey of Unbanked and Underbanked Households (2013--2023), I document that self-employment rates are significantly higher among branch banking users (9.95\%) compared to mobile-only users (7.19\%). I develop a dynamic structural model of joint banking mode and employment choice estimated via the Arcidiacono-Miller CCP approach with finite dependence and unobserved heterogeneity. Using BIC for model selection, I identify three latent types: relationship borrowers (9\%), tech-savvy entrepreneurs (29\%), and potential entrepreneurs (62\%). Counterfactual simulations show that a 50\% reduction in branch access reduces aggregate self-employment by 6\%, with this effect robust across model specifications ($K=1,2,3,4$ types). The results suggest that while branch closures negatively affect self-employment, the effects are more modest than homogeneous models predict, as they account for heterogeneity in entrepreneurial propensity across the population.
...
\begin{itemize}
    \item \textbf{Banking mode} $b_t \in \{U, M, B\}$ (unbanked, mobile, branch)
    \item \textbf{Employment status} $d_t \in \{W, S, N\}$ (wage, self-employed, not working)
\end{itemize}

Individuals are forward-looking and maximize expected discounted lifetime utility. The key dynamic elements are: (i) self-employment experience accumulates and raises future self-employment returns; (ii) switching banking modes incurs a one-time utility cost; and (iii) local banking infrastructure evolves according to a Markov process.

\subsection{State Space}

The state vector for individual $i$ in CBSA $j$ at time $t$ is:
\begin{equation}
s_{ijt} = (X_{it}, E_{it}, b_{t-1}, Z_{jt})
\end{equation}
where:
\begin{itemize}
    \item $X_{it}$ = time-varying demographics (age category, potentially time-varying income)
    \item $E_{it} \in \{0, 1, 2, ...\}$ = accumulated self-employment experience (years)
    \item $b_{t-1} \in \{U, M, B\}$ = lagged banking mode (for switching costs)
    \item $Z_{jt}$ = CBSA-level variables (branch density, broadband penetration)
\end{itemize}

Education and race are time-invariant and enter the flow utility directly.

\subsection{State Transitions}

The law of motion for the state vector has three components:

\textbf{1. Individual state evolution:}
\begin{align}
X_{i,t+1} &= f_X(X_{it}) \quad \text{(deterministic aging)} \\
E_{i,t+1} &= E_{it} + \mathbf{1}[d_t = S] \quad \text{(experience accumulates with SE)}
\end{align}

Age transitions deterministically across categories. Self-employment experience increments by one for each period spent self-employed, and is otherwise unchanged.

\textbf{2. Banking mode persistence:}
\begin{equation}
b_t \rightarrow b_{t-1,\text{next period}}
\end{equation}

The lagged banking mode updates to reflect the current choice, generating state dependence through switching costs.

\textbf{3. CBSA-level Markov transitions:}

I estimate the transition process for CBSA-level variables from FDIC Summary of Deposits data:
\begin{equation}
Z_{j,t+1} \sim F_Z(\cdot | Z_{jt})
\end{equation}

Branch density follows a first-order Markov process with persistence parameter $\rho_Z \approx 0.95$, reflecting the slow evolution of banking infrastructure. Broadband penetration trends upward with CBSA-specific growth rates.

\subsection{Flow Utility}

The per-period utility of choosing banking mode $b$ and employment status $d$ is:
\begin{align}
u(b,d,s_{ijt}) &= \alpha_{bd} + X_{it}'\beta_{bd} + \gamma_E \cdot E_{it} \cdot \mathbf{1}[d = S] \nonumber \\
&\quad + \gamma_C \cdot \mathbf{1}[d = S] \cdot CreditAccess(b, Z_{jt}) \nonumber \\
&\quad - \kappa \cdot \mathbf{1}[b \neq b_{t-1}] + \varepsilon_{ijt}^{bd}
\end{align}

where:
\begin{itemize}
    \item $\alpha_{bd}$ = alternative-specific constants
    \item $\gamma_E$ = return to self-employment experience
    \item $\gamma_C$ = value of credit access for self-employment
    \item $\kappa$ = banking mode switching cost
    \item $\varepsilon_{ijt}^{bd}$ = Type 1 extreme value taste shocks (i.i.d. across alternatives and time)
\end{itemize}
...
The parameter $\gamma_C$ captures how credit access affects self-employment returns. The $\delta$ parameters map infrastructure to credit availability through each banking channel.

\subsection{Value Function and Bellman Equation}

Individuals maximize expected discounted lifetime utility with discount factor $\beta$. The value function satisfies:
\begin{equation}
V(s_{ijt}) = \max_{b,d} \left\{ u(b,d,s_{ijt}) + \varepsilon_{ijt}^{bd} + \beta \mathbb{E}\left[ V(s_{ij,t+1}) \,|\, b, d, s_{ijt} \right] \right\}
\end{equation}

The expectation is taken over:
\begin{itemize}
    \item The Markov transition of CBSA-level variables $Z_{j,t+1} | Z_{jt}$
    \item Next period's taste shocks $\varepsilon_{ij,t+1}$
\end{itemize}

Given the deterministic evolution of individual states conditional on choices, the continuation value depends on:
\begin{equation}
\mathbb{E}\left[ V(s_{ij,t+1}) \,|\, b, d, s_{ijt} \right] = \int V(X_{i,t+1}, E_{it} + \mathbf{1}[d=S], b, Z') \, dF_Z(Z' | Z_{jt})
\end{equation}

With Type 1 extreme value errors, the expected value (integrated over taste shocks) has the well-known logsum form:
\begin{equation}
\bar{V}(s) = \log \left( \sum_{b,d} \exp\left( u(b,d,s) + \beta \mathbb{E}[\bar{V}(s') | b,d,s] \right) \right) + \gamma_E
\end{equation}
where $\gamma_E \approx 0.5772$ is Euler's constant.

\subsection{Conditional Choice Probabilities}

The probability of choosing $(b,d)$ given state $s$ is:
\begin{equation}
P(b,d | s) = \frac{\exp\left( u(b,d,s) + \beta \mathbb{E}[\bar{V}(s') | b,d,s] \right)}{\sum_{b',d'} \exp\left( u(b',d',s) + \beta \mathbb{E}[\bar{V}(s') | b',d',s] \right)}
\end{equation}

\subsection{Estimation via CCP Methods}

I estimate the model using the two-step CCP approach of \citet{hotz1993} as extended by \citet{arcidiacono2011}. The key insight is that continuation values can be expressed in terms of observable CCPs, avoiding the need to solve the full dynamic program.

\textbf{Step 1: Estimate CCPs.} From the data, I estimate conditional choice probabilities $\hat{P}(b,d|s)$ for each state cell defined by CBSA $\times$ year $\times$ age $\times$ education.

\textbf{Step 2: Finite Dependence and the Renewal Action.}

The Arcidiacono-Miller approach exploits \textit{finite dependence}: if there exists a ``renewal'' action that resets the payoff-relevant state, then continuation value differences can be computed without solving the full Bellman equation.

I designate $(b,d) = (B, W)$ (branch banking $\times$ wage employment) as the renewal action. This choice resets the relevant state variables in the following sense:
\begin{itemize}
    \item Self-employment experience $E$ does not accumulate (no return to experience differential going forward)
    \item Branch banking is the baseline mode (no switching cost from the modal choice)
    \item Future credit access is anchored at the branch-banking level
\end{itemize}

Under one-period finite dependence, the difference in continuation values between any action $(b,d)$ and the renewal action $(B,W)$ can be written as:
\begin{align}
&\beta \mathbb{E}[\bar{V}(s') | b,d,s] - \beta \mathbb{E}[\bar{V}(s') | B,W,s] \nonumber \\
&= \beta \sum_{s'} \left[ F(s'|b,d,s) - F(s'|B,W,s) \right] \bar{V}(s')
\end{align}

Since $\bar{V}(s') = \log\sum_{b',d'} \exp(v_{b'd'}(s')) + \gamma_E$, and the logsum can be computed from first-stage CCPs via:
\begin{equation}
\bar{V}(s') = -\log P(B,W|s') + v_{BW}(s') + \gamma_E
\end{equation}

this allows me to express continuation value differences as functions of CCPs and flow utilities.

\textbf{Step 3: Pseudo-Likelihood Estimation.}

Given the CCP-based representation of continuation values, I estimate structural parameters by maximizing:
\begin{equation}
\mathcal{L}(\theta) = \sum_i \sum_t \log P(b_{it}, d_{it} | s_{it}; \theta, \hat{P})
\end{equation}

where $\hat{P}$ denotes first-stage CCP estimates. Standard errors are computed via bootstrap to account for first-stage estimation error.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Structural Results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Phase 1: Static Multinomial Logit}
...
\subsection{Phase 2: Dynamic CCP Estimation}

Table \ref{tab:structural_phase2} presents key structural parameters from the dynamic CCP estimation using the Arcidiacono-Miller approach with finite dependence. The model designates Branch $\times$ Wage employment as the renewal action, which resets the payoff-relevant state and allows computation of continuation value differences without solving the full Bellman equation.

\begin{table}[H]
\centering
\caption{Dynamic Structural Parameters: CCP Estimation}
...
$\gamma_{dynamic}$ (SE experience return) & 0.559*** & (0.082) & Returns to SE experience \\
$\kappa$ (switching cost) & $-6.179$*** & (0.095) & Banking mode persistence \\
\midrule
\multicolumn{4}{l}{\textit{Credit Access Parameters}} \\
$\gamma_{broadband \times mobile}$ & 0.164** & (0.083) & Broadband $\rightarrow$ mobile adoption \\
$\gamma_{broadband \times SE}$ & $-0.120$* & (0.067) & Broadband effect on SE \\
\midrule
\multicolumn{4}{l}{\textit{Demographic Interactions}} \\
$\gamma_{age30-44 \times SE}$ & 0.154* & (0.090) & Prime working age SE premium \\
$\gamma_{college \times SE}$ & 0.129 & (0.086) & Education effect on SE \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: Estimates from dynamic CCP estimation with finite dependence. Renewal action: Branch $\times$ Wage employment. Discount factor $\beta = 0.90$. 436 CBSA $\times$ year $\times$ demographic cells, N = 3,924 cell-alternative observations. Standard errors clustered at CBSA level. $^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$.
\end{tablenotes}
\end{threeparttable}
\end{table}

The dynamic parameters reveal two key findings. First, the return to self-employment experience ($\gamma_{dynamic} = 0.559$) is substantial and highly significant, indicating that entrepreneurial human capital accumulates over time and raises future self-employment returns. This creates path dependence: individuals who enter self-employment early accumulate experience that makes continued self-employment increasingly attractive.

Second, the switching cost ($\kappa = -6.18$) is large and negative, indicating strong persistence in banking mode choice. Once individuals adopt a particular banking mode, they are unlikely to switch, even when circumstances change. This has important implications for branch closures: displaced branch users who switch to mobile banking face a permanent utility loss beyond the immediate change in credit access.

The credit access parameters show that broadband increases mobile banking adoption ($\gamma_{broadband \times mobile} = 0.164$), but areas with higher broadband have slightly lower self-employment rates ($\gamma_{broadband \times SE} = -0.120$), reflecting selection of entrepreneurs into branch banking. The difference between branch and mobile credit access ($\gamma_{broadband \times SE} - \gamma_{broadband \times mobile} = -0.284$) captures the credit access disadvantage of mobile banking for entrepreneurship.

\subsection{Unobserved Heterogeneity}

Following \citet{arcidiacono2011}, I extend the model to incorporate unobserved heterogeneity via a finite mixture. The number of types $K$ is selected using the Bayesian Information Criterion (BIC), which penalizes model complexity more heavily than AIC---an important consideration when types are latent. Table \ref{tab:bic_selection} presents the model selection results.

\begin{table}[H]
\centering
\caption{Model Selection: BIC Comparison Across Number of Types}
...
& $K=1$ & $K=2$ & $K=3$ & $K=4$ \\
\midrule
Log-likelihood & $-6,897$ & $-6,777$ & $-6,748$ & $-6,758$ \\
Parameters & 13 & 16 & 20 & 22 \\
BIC & 13,901 & 13,687 & \textbf{13,661} & 13,698 \\
AIC & 13,819 & 13,587 & 13,535 & 13,560 \\
\midrule
CF effect (50\% closure) & $-6.2\%$ & $-9.5\%$ & $-6.1\%$ & -- \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: BIC $= -2\ln\mathcal{L} + p \cdot \ln(N)$ where $p$ is parameters and $N=3,924$ cell-alternative observations. Bold indicates minimum BIC. Counterfactual effects are weighted averages across types.
\end{tablenotes}
\end{threeparttable}
\end{table}

BIC selects $K=3$ types. Importantly, the counterfactual predictions are relatively stable across specifications (ranging from $-6.1\%$ to $-9.5\%$), suggesting the substantive conclusions are robust to the choice of $K$.

The three BIC-selected types are economically interpretable:

\begin{itemize}
    \item \textbf{Type 1 (Relationship Borrowers, 9.1\%)}: Strong preference for branch banking, negative dynamic returns to SE experience ($\gamma_{dynamic} = -0.30$). These are individuals who rely heavily on in-person banking relationships but have low entrepreneurial propensity.

    \item \textbf{Type 2 (Tech-Savvy Entrepreneurs, 29.0\%)}: Moderate SE experience returns ($\gamma_{dynamic} = 0.57$), high sensitivity to credit access ($\gamma_{bb \times SE} = -0.16$). Can partially substitute mobile for branch banking.

    \item \textbf{Type 3 (Potential Entrepreneurs, 61.9\%)}: Highest returns to SE experience ($\gamma_{dynamic} = 0.74$), moderate credit sensitivity ($\gamma_{bb \times SE} = -0.12$). The largest group, representing individuals with entrepreneurial potential who are affected by banking access.
\end{itemize}

Table \ref{tab:unobs_het} presents the full type-specific parameter estimates.

\begin{table}[H]
\centering
\caption{Type-Specific Parameters (BIC-Selected $K=3$)}
\label{tab:unobs_het}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
& Type 1 & Type 2 & Type 3 & Weighted \\
& (Relationship) & (Tech-Savvy) & (Potential) & Average \\
\midrule
Population share & 9.1\% & 29.0\% & 61.9\% & -- \\
\midrule
$\gamma_{dynamic}$ (SE experience) & $-0.301$ & 0.570 & 0.743 & 0.559 \\
$\kappa$ (switching cost) & 4.38 & 0.00 & $-0.23$ & 0.26 \\
$\gamma_{bb \times SE}$ (credit access) & $-0.01$ & $-0.16$ & $-0.12$ & $-0.12$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: Parameters estimated via Arcidiacono-Miller CCP method with type-probability weights based on observable characteristics. Type 2's switching cost is zero due to collinearity.
\end{tablenotes}
\end{threeparttable}
\end{table}

The type-specific parameters reveal important heterogeneity. Type 1 (Relationship Borrowers) has a large positive switching cost (4.38), indicating strong attachment to branch banking, but negative returns to SE experience---these individuals prefer stable wage employment. Type 3 (Potential Entrepreneurs) has the highest returns to SE experience (0.74) and a small negative switching cost, indicating flexibility in banking mode choice.

The weighted average counterfactual effect of 50\% branch closure is $-6.1\%$, smaller than the homogeneous model estimate ($-19.3\%$). This difference reflects compositional effects: the homogeneous model imposes identical parameters on all types, overstating the effect for Type 1 individuals who have low entrepreneurial propensity regardless of banking access.
...
Table \ref{tab:counterfactual} presents counterfactual predictions from the dynamic structural model. The dynamic framework captures three channels through which branch closures affect self-employment:

\begin{enumerate}
    \item \textbf{Static credit access}: Immediate loss of branch-based relationship lending
    \item \textbf{Dynamic experience effects}: Reduced self-employment entry leads to less experience accumulation
    \item \textbf{Compounding over time}: Lower experience reduces future self-employment returns, creating a downward spiral
\end{enumerate}

Under 50\% branch closure, I assume 80\% of displaced branch users switch to mobile banking and 20\% become unbanked. The self-employment adjustment incorporates both static credit access differentials and dynamic experience effects.

\begin{table}[H]
\centering
\caption{Dynamic Counterfactual Policy Simulations}
\label{tab:counterfactual}
\begin{threeparttable}
\begin{tabular}{lcccc}
\toprule
Scenario & SE Rate & Change & \% Change & Dynamic Multiplier \\
\midrule
\textbf{Baseline} & 11.40\% & -- & -- & -- \\
\midrule
\multicolumn{5}{l}{\textit{Branch Closures (Dynamic Model)}} \\
\quad 50\% closure & 9.20\% & $-2.20$ pp & $\mathbf{-19.3\%}$ & 2.45$\times$ \\
\midrule
\multicolumn{5}{l}{\textit{Comparison: Static Model}} \\
\quad 50\% closure (static) & 10.27\% & $-0.88$ pp & $-7.9\%$ & 1.00$\times$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: Dynamic counterfactual predictions based on Arcidiacono-Miller CCP estimates with $\beta = 0.90$. Branch closure assumes 80\% switch to mobile, 20\% become unbanked. Static adjustment factor: $\exp(\gamma_{bb,SE} - \gamma_{bb,mobile}) = 0.74$. Dynamic adjustment factor: static $\times$ $\exp(\gamma_{dynamic} \times (-0.5)) = 0.56$. Dynamic multiplier shows the ratio of dynamic to static effects.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{enumerate}
    \item \textbf{Dynamic effects substantially amplify branch closure impacts}: The dynamic model predicts a 19.3\% decline in self-employment from 50\% branch closure, compared to only 7.9\% in the static model---a \textbf{dynamic multiplier of 2.45}. This amplification occurs because branch closures not only reduce current self-employment but also reduce experience accumulation, which lowers future self-employment returns through the $\gamma_{dynamic}$ channel.

    \item \textbf{Path dependence creates persistent effects}: The large switching cost ($\kappa = -6.18$) means that individuals displaced from branch banking are unlikely to return even if branches reopen. Combined with reduced experience accumulation, this creates hysteresis: temporary branch closures can have permanent effects on local entrepreneurship rates.

    \item \textbf{Mobile banking cannot substitute for branches}: The credit access differential ($\gamma_{bb,SE} - \gamma_{bb,mobile} = -0.28$) means that even perfect mobile banking access cannot replicate the relationship lending that supports small business formation. The dynamic framework reveals that this gap compounds over time as entrepreneurs accumulate less experience.
\end{enumerate}
...
This paper investigates whether mobile banking can substitute for traditional branch banking in supporting self-employment, using data from the FDIC National Survey of Unbanked and Underbanked Households (2013--2023) combined with a dynamic structural model of joint banking mode and employment choice.
...
Second, the dynamic structural estimates reveal important sources of state dependence in both employment and banking choices. The return to self-employment experience ($\gamma_{dynamic} = 0.56$) is substantial, indicating that entrepreneurial human capital accumulates over time. The banking mode switching cost ($\kappa = 6.18$) is large, creating strong persistence in banking relationships. Together, these parameters imply that branch closures have compounding effects: reduced credit access lowers current self-employment, which reduces experience accumulation, which further lowers future self-employment returns.

Third, employing BIC-based model selection following \cite{arcidiacono2011}, we identify three distinct unobserved types with heterogeneous responses to branch closures. The BIC criterion selects K=3 types over simpler (K=1,2) and more complex (K=4) specifications. The largest group---Potential Entrepreneurs comprising 61.9\% of the population---shows moderate sensitivity to branch access and represents individuals on the margin of self-employment entry. Relationship Borrowers (9.1\%) are highly dependent on branch relationships for credit access, while Tech-Savvy Entrepreneurs (29.0\%) show greater resilience through digital banking alternatives. The counterfactual analysis indicates that a 50\% branch closure would reduce aggregate self-employment by approximately 6\%, with this effect robust across model specifications (ranging from 6.1\% to 9.5\% across different values of K).

These findings have important policy implications. The 6\% aggregate effect of substantial branch closures on self-employment is economically meaningful but moderate, suggesting that mobile banking provides partial---though incomplete---substitution for traditional branch services. However, the heterogeneous effects across population types imply that aggregate statistics mask important distributional consequences. Relationship Borrowers, who depend most heavily on branch-based lending relationships, bear disproportionate costs from branch closures. As bank branches continue to close---particularly in lower-income and minority communities---policies that preserve branch access or develop alternative channels for relationship-based small business lending may be essential for maintaining pathways to self-employment for these populations.
