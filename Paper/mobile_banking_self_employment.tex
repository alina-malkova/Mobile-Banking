...
\begin{abstract}
\noindent This paper investigates whether mobile banking can substitute for traditional branch banking in supporting self-employment. Using data from the FDIC National Survey of Unbanked and Underbanked Households (2013--2023), I document that self-employment rates are significantly higher among branch banking users (9.95\%) compared to mobile-only users (7.19\%). I develop a dynamic structural model of joint banking mode and employment choice estimated via the Arcidiacono-Miller CCP approach with finite dependence. The key dynamic elements include self-employment experience accumulation, banking mode switching costs, and forward-looking behavior. The structural estimates reveal substantial returns to self-employment experience ($\gamma = 0.56$) and large banking mode switching costs ($\kappa = 6.18$), indicating strong state dependence in both employment and banking choices. Counterfactual simulations incorporating dynamic effects show that a 50\% reduction in branch access reduces aggregate self-employment by 19.3\%---substantially larger than static models predict---because branch closures reduce not only current self-employment but also future experience accumulation and entrepreneurial human capital. These findings suggest that policies promoting mobile banking cannot offset the compounding negative effects of branch closures on entrepreneurship.
...
Third, dynamic counterfactual simulations reveal that branch closures have much larger effects than static models suggest. A 50\% reduction in branch access reduces aggregate self-employment by \textbf{19.3\%}---more than twice the 7.9\% predicted by static models. This ``dynamic multiplier'' of 2.45 arises because the model captures not only the immediate credit access loss but also the long-run erosion of entrepreneurial human capital as fewer individuals accumulate self-employment experience.

These findings have important policy implications. As bank branches continue to close---particularly in lower-income and minority communities---policymakers should recognize that the full costs extend well beyond the immediate effects on credit access. The dynamic nature of entrepreneurship means that temporary disruptions to banking relationships can have persistent effects on local self-employment rates through reduced human capital accumulation. Mobile banking and broadband investment cannot offset these effects because they cannot replicate the relationship lending that supports small business formation. Policies that preserve branch access in underserved communities may be essential to maintaining pathways to self-employment for future generations of entrepreneurs.
